import { _decorator, Component, Node , input, Input, EventMouse, Sprite, Vec3, view, Collider2D, Contact2DType} from 'cc';
...
export class Hook extends Component {
    ...
    private startAngle:number = -225;
    @property
    private endAngle:number = -70;
    @property
    private rotateSpeed:number;
    @property
    private hookoutSpeed:number;
    @property
    ...
    start() {
        ...
        //添加侦听器
        input.on(Input.EventType.MOUSE_DOWN,this.hookOut,this);
    ...
    hookOut(event:EventMouse){
        //左键被按下,出钩
        if(event.getButton() == 0 && !this.isHookOut){
            this.isHookOut = true;
            this.isBack = false;
