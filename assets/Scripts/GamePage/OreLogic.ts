...
export class OreLogic extends Component {
    ...
    @property({
        group: { name: "显示方案" },
        tooltip: "Add imgs into List if You want to change img of this node",
    })
    ...
    @property({
        group: { name: "运动方案" },
        tooltip:
            "Modify the Start and End as well Add RigidBody2D if You want to move this node",
    })
    ...
    public movingAround(deltaTime: number) {
        ...
        this._deltaPos.x =
            Math.cos(this._direction) * this.movingSpeed * deltaTime * 10;
        this._deltaPos.y =
            Math.sin(this._direction) * this.movingSpeed * deltaTime * 10;
        ...
        function randomDirection(curDirection: number): number {
            ...
            let direction: number =
                curDirection + Math.random() * 0.5 * Math.PI - 0.25 * Math.PI;
    ...
    private isStudentCollison(curPos: Vec3) {
        ...
        if (
            curPos.x < 30 ||
            curPos.x > this.contentSize.width - 30 ||
            curPos.y < 30 ||
            curPos.y > this.contentSize.height - 30
        ) {
            return true;
        } else return false;
    }
        ...
        switch (this.type) {
            ...
            case OreType.RandomBag:
                ...
                this.value =
                    ...
                    Math.round(
                        (Math.random() *
                            (this.randomMaxValue - this.randomMinValue)) /
                            10
                    ) *
                        10;
